...
function renameVariables(baseNode, namesToRename) {
    ...
    (0, walkers_1.recursive)(baseNode, new Set(), {
        VariablePattern(node, inactive, c) {
            // for declarations
            const name = node.name;
...
function getLocalsInScope(node) {
    ...
    for (const stmt of node.body) {
        if (stmt.type === 'VariableDeclaration') {
            const stmtNode = stmt;
            const name = stmtNode.declarations[0].id.name;
            locals.add(name);
        }
        else if (stmt.type === 'FunctionDeclaration') {
            const stmtNode = stmt;
...
const compilers = {
    ...
    ReturnStatement(node, indexTable, insertFlag) {
        node = node;
    ...
    ForStatement(node, indexTable, insertFlag) {
        throw Error('Unsupported operation');
    },
    ...
    ObjectExpression(node, indexTable, insertFlag) {
        throw Error('Unsupported operation');
    },
    ...
    Property(node, indexTable, insertFlag) {
        throw Error('Unsupported operation');
    },
    DebuggerStatement(node, indexTable, insertFlag) {
        throw Error('Unsupported operation');
    }
};
